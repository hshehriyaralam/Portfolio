  ...
  return (
    ...
    <nav  className={`w-full fixed px-5 lg:px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm" : ""}`}>
        ...
        <div className='flex items-center gap-4 ' >
          <button>
            <Image src={assets.moon_icon} alt='Toggle' className='w-6' />
          </button>
            <a href="#contact"  className={`hidden lg:flex items-center gap-3 px-10 py-2.5 border border-gray-500 rounded-full ml-4 ${Styles.FontOvo}`} >Contact <Image alt='arrow_icons'  src={assets.arrow_icon}  
            className='w-3' /> </a>

            <button   className='block md:hidden ml-3  cursor-pointer' onClick={openMenu}>
            <Image src={assets.menu_black} alt='Menu_Button' className='w-6' />
          </button>
        </div>
